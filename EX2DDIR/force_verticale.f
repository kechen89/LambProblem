...
c *** fin ajout D.K.
      ...
      double precision      U(0:NN),W(0:NN),T(0:NN),S
      double precision      CR,PR,PPS,PSP,A1,A2,DELAY,FP
...
c  A2   =-HALF*(TWOPI*FP)**2
      ...
      S    =-A1*DELAY*DEXP(A2*DELAY*DELAY)
      IF(DISPV.EQ.1) A1 = TWO*A1
...
c pour verif avec source du code diff. finies
c        call system('/bin/rm -f source')
...
***************  WRITE RUN INF.ON UNIT 9
      ...
      WRITE(9,'(A19,F11.3,A8)')' DENSITY          =',RHO  ,' G/CM**3'
      ...
      WRITE(9,'(A19,F11.3,A5)')' SOURCE AT T=0    =',S         ,'     '
      DO 1 I=1,2
...
c *** modif D.K. ecriture sismogrammes normalises et au format SEP
	...
	do it=1,ntstp
c inversion du signe pour compatibilite convention heiner
...
c      write(*,*) 'Ecriture sur disk au format SEP...'
c ASCII output (by M. Käser)
      write(*,*) '  (fichiers Ux_file_ascii et Uz_file_ascii)'

      write(*,*) 'Pas de normalisation mais multiplication par ',fac2
      do irec=1,nrec
              do it=1,ntstp
...
c effacer vieux fichiers
      call system('rm -f Ux_file_an ; rm -f Uz_file_an')

c ecriture au format SEP (binaire) du deplacement horizontal
c      open(unit=11,file='Ux_file_an',status='unknown',
c     .				access='direct',recl=ntstp*4)
c      do irec=1,nrec
c      write(11,rec=irec) (horiz(it,irec),it=1,ntstp)
c      enddo
c      close(11)

c ecriture au format SEP (binaire) du deplacement vertical
c      open(unit=11,file='Uz_file_an',status='unknown',
c     .				access='direct',recl=ntstp*4)
c      do irec=1,nrec
c      write(11,rec=irec) (vert(it,irec),it=1,ntstp)
c      enddo
c      close(11)

c ASCII output (by M. Käser)
      open(unit=11,file='Ux_file_ascii',status='unknown')
        do irec = 1,nrec
	    do it = 1,ntstp
             write(11,*) horiz(it,irec)
          enddo     
        enddo
      close(11)

      open(unit=11,file='Uz_file_ascii',status='unknown')
        do irec = 1,nrec
	    do it = 1,ntstp
             write(11,*) vert(it,irec)
          enddo     
        enddo
      close(11)
...
*************** SOURCE TIME HIST. = 2-LOOP RICKER
      double precision T,A1,A2,DELAY,TAU,ZERO,HALF,TEMP
      INTEGER DISPV
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
...
c ---------------------------------------
c --- Choix source, sortie en vitesse ---
c ---------------------------------------

c *** Gaussienne, sortie en vitesse
c      sour = a1*( tau )*dexp(temp)

c *** derivee premiere de Gaussienne, sortie en vitesse
c      SOUR = A1*( HALF + TEMP )*DEXP(TEMP)

c *** Ricker (derivee seconde de gaussienne), sortie en vitesse
	 sour = a1*a2*tau*DEXP(TEMP)*(3.d0 + 2.d0*temp)

c ---------------------------------------
c ---    Fin choix source en vitesse  ---
c ---------------------------------------
...
************************************************************************
      ...
      INTEGER DISPV
      double precision TAU,X,Z,C2I,R2,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
...
************************************************************************
      ...
      INTEGER DISPV
      double precision TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
...
************************************************************************
      ...
      INTEGER DISPV
      double precision TAU,TIME,SOUR,A1,A2,DELAY
...
************************************************************************
      ...
      INTEGER DISPV
      double precision TAU,TIME,SOUR,A1,A2,DELAY,CP2I,CS2I,X
...
c &&&&&&&& D.K. plante a la surface, remplacer sur Moka par : &&&&&

        double precision FUNCTION myD01AHF(A,B,RA,NP,RE,F,NL,IF)
        ...
        double precision w(10000),iw(10000)
...
c &&&& enlever ce commentaire sur moka
c       call d01ajf(F,a,b,errabs,ra,result,errest,W, LW, IW, LIW, IFAIL)
...
c &&&&& Compiler avec : f77 -o xforce force_verticale.f -l nag
